...
public class CorrelationCalculator {
    ...
    private List<AirModel> list;
    private List<AirView> outList = new LinkedList<>();
     ...
     */
    public CorrelationCalculator with(List<AirModel> list) {
        this.list = list;
    ...
    private void convertListToRealMatrix() {
        ...
        for (AirModel element : list) {
            matrix.setEntry(i, 0, element.getFlags());
            matrix.setEntry(i, 1, element.getCatalog());
            matrix.setEntry(i, 2, element.getElevation());
            matrix.setEntry(i, 3, element.getFrequency());
            matrix.setEntry(i, 4, element.getLength());
            i++;
        }
    }

    public List<AirView> getOutList() {
        for (int i = 0; i < 5; i++) {
